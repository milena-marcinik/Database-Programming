...
# USUWANIE
print(session.query(User).all()) # wszyscy userzy
...
session.commit()
print("po usunieciu:")
print(session.query(User).all())  # no nie ma Jacka

print("adresy email Jacka")
print(session.query(Address).all())  # dalej istnieja
# rekordy z tabeli "addresses" nadal sa w bazie danych
# domyślnie sql alchemy nie używa kaskady przy usuwaniu
# jeżeli chcemy to zmienić, to musimy to jawnie wskazać tam przy tworzeniu w klasie User relacji do adresow